...
一个基于 STM32H743 微控制器的嵌入式引导加载程序项目，集成了 RT-Thread 实时操作系统。

## 📋 项目简介

DiffBoot 是一个功能丰富的嵌入式引导加载程序，支持固件更新、差分升级等功能。项目采用模块化设计，便于扩展和维护。

## ✨ 主要特性

- **STM32H743 支持**: 基于 ARM Cortex-M7 内核的高性能微控制器
- **RT-Thread RTOS**: 集成轻量级实时操作系统
- **差分升级**: 支持高效的固件差分更新
...
- **IDE**: 
  - [EIDE](https://github.com/EmbeddedXplorer/EIDE) (推荐)
  - VS Code
  - Keil MDK
- **调试工具**: J-Link / ST-Link
...
uint16_t crc = algo_crc16(data, sizeof(data));

// 初始化环形缓冲区
uint8_t buffer_pool[256];
struct rt_ringbuffer rb;
rt_ringbuffer_init(&rb, buffer_pool, sizeof(buffer_pool));

// 写入数据
rt_ringbuffer_put(&rb, data, sizeof(data));
```
