 ...
 */
INLINE static inline void rt_list_init(rt_list_t *l)
{ l->next = l->prev = l; }
 ...
 */
INLINE static inline void rt_list_insert_after(rt_list_t *l, rt_list_t *n)
{
    l->next->prev = n;
    n->next       = l->next;
    l->next       = n;
 ...
 * @param l list to insert it
 */
INLINE static inline void rt_list_insert_before(rt_list_t *l, rt_list_t *n)
{
    l->prev->next = n;
    n->prev       = l->prev;
    l->prev       = n;
 ...
 */
INLINE static inline void rt_list_remove(rt_list_t *n)
{
    n->next->prev = n->prev;
    n->prev->next = n->next;
    n->next = n->prev = n;
 ...
 */
INLINE static inline int rt_list_isempty(const rt_list_t *l)
{ return l->next == l; }
 ...
 */
INLINE static inline unsigned int rt_list_len(const rt_list_t *l)
{
    unsigned int len   = 0;
    const rt_list_t *p = l;
    while (p->next != l) {
