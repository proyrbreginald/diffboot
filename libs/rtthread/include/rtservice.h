 ...
 */
INLINE static inline void rt_list_insert_after(rt_dlist_t *l, rt_dlist_t *n)
{
    l->next->prev = n;
    n->next       = l->next;
    l->next       = n;
 ...
 * @param l list to insert it
 */
INLINE static inline void rt_list_insert_before(rt_dlist_t *l, rt_dlist_t *n)
{
    l->prev->next = n;
    n->prev       = l->prev;
    l->prev       = n;
 ...
 */
INLINE static inline void rt_list_remove(rt_dlist_t *n)
{
    n->next->prev = n->prev;
    n->prev->next = n->next;
    n->next = n->prev = n;
 ...
 */
INLINE static inline int rt_list_isempty(const rt_dlist_t *l)
{ return l->next == l; }
 ...
 */
INLINE static inline unsigned int rt_list_len(const rt_dlist_t *l)
{
    unsigned int len   = 0;
    const rt_dlist_t *p = l;
    while (p->next != l) {
        p = p->next;
        len++;
    }
 ...
 * @pos    the rt_dlist_t * to use as a loop cursor.
 ...
 * entry
 * @pos    the rt_dlist_t * to use as a loop cursor.
 * @n      another rt_dlist_t * to use as temporary storage
 * @head   the head for your list.
 */
...
INLINE static inline unsigned int rt_slist_len(const rt_slist_t *l)
{
    unsigned int len       = 0;
...
INLINE static inline rt_slist_t *rt_slist_remove(rt_slist_t *l, rt_slist_t *n)
{
    /* remove slist head */
...
INLINE static inline rt_slist_t *rt_slist_first(rt_slist_t *l)
{ return l->next; }

INLINE static inline rt_slist_t *rt_slist_tail(rt_slist_t *l)
{
    while (l->next) l = l->next;
    return l;
}

INLINE static inline rt_slist_t *rt_slist_next(rt_slist_t *n)
{ return n->next; }
