...
static int chunk_unpack_header_size(
    struct detools_apply_patch_chunk_t *self_p,
    struct detools_apply_patch_size_t *size_state_p, int *size_p)
{
    int res;
    uint8_t byte;
...
static int patch_reader_none_init(
    struct detools_apply_patch_patch_reader_t *self_p, size_t patch_size)
{
    struct detools_apply_patch_patch_reader_none_t *none_p;
    ...
    size_t *size_p)
{
    ...
    {
        /* Get available data. */
        pres = heatshrink_decoder_poll(heatshrink_p->decoder_p, buf_p, left,
                                       &size);
...
static int get_decompressed_data(
    struct detools_apply_patch_patch_reader_lzma_t *lzma_p, uint8_t *buf_p,
    size_t size)
{
    int res;
...
static int prepare_input_buffer(
    struct detools_apply_patch_patch_reader_t *self_p)
{
...
static int prepare_output_buffer(
    struct detools_apply_patch_patch_reader_t *self_p, size_t size)
{
    struct detools_apply_patch_patch_reader_lzma_t *lzma_p;
    uint8_t *output_p;
...
static int patch_reader_lzma_init(
    struct detools_apply_patch_patch_reader_t *self_p)
{
    lzma_ret ret;
    struct detools_apply_patch_patch_reader_lzma_t *lzma_p;
    ...
    struct detools_apply_patch_patch_reader_crle_t *crle_p)
{
    ...
    res = unpack_usize(&crle_p->kind.scattered.size, self_p->patch_chunk_p,
                       &size);
    ...
    size_t *size_p)
{
    ...
    do
    {
        ...
        case detools_crle_state_scattered_data_t:
            res = patch_reader_crle_decompress_scattered_data(self_p, crle_p,
                                                              buf_p, size_p);
            break;
...
static int patch_reader_crle_init(
    struct detools_apply_patch_patch_reader_t *self_p)
{
...
static int patch_reader_dump(struct detools_apply_patch_patch_reader_t *self_p,
                             int compression, detools_state_write_t state_write)
{
    (void)self_p;
    (void)state_write;
...
static int patch_reader_restore(
    struct detools_apply_patch_patch_reader_t *self_p,
    struct detools_apply_patch_patch_reader_t *dumped_p,
    struct detools_apply_patch_chunk_t *patch_chunk_p, int compression,
...
static int common_process_size(
    struct detools_apply_patch_patch_reader_t *patch_reader_p, size_t to_pos,
    size_t to_size, int *size_p)
{
    int res;
...
size_t detools_apply_patch_get_patch_offset(
    struct detools_apply_patch_t *self_p)
{
...
int detools_apply_patch_finalize(struct detools_apply_patch_t *self_p)
{
    ...
    return (apply_patch_common_finalize(res, &self_p->patch_reader,
                                        self_p->to_size));
}
...
static int in_place_all_steps_completed(
...
static int in_place_is_step_completed(
    struct detools_apply_patch_in_place_t *self_p, bool *res_p)
{
    int res;
    int completed_step;
                                 ...
                                 size_t memory_size, size_t from_size)
{
    ...
    for (i = 0; i < number_of_segments; i++)
    {
        ...
        while (offset < self_p->segment_size)
        {
            ...
            res =
                in_place_mem_read(self_p, &buf[0], read_address + offset, size);
            ...
            res = in_place_mem_write(self_p, write_address + offset, &buf[0],
                                     size);
    ...
    struct detools_apply_patch_in_place_t *self_p)
{
    ...
    res =
        chunk_unpack_header_size(&self_p->chunk, &self_p->size, &segment_size);
    ...
    struct detools_apply_patch_in_place_t *self_p)
{
    ...
    if (to_size > 0)
    {
        res = in_place_shift_memory(self_p, self_p->memory_size,
                                    self_p->from_size);
...
static int in_place_process_diff_size(
    struct detools_apply_patch_in_place_t *self_p)
{
    return (
        in_place_process_size(self_p, detools_apply_patch_state_diff_data_t));
}

static int in_place_process_diff_data(
    struct detools_apply_patch_in_place_t *self_p)
{
    return (
...
static int in_place_process_extra_size(
    struct detools_apply_patch_in_place_t *self_p)
{
    return (
...
static int in_place_process_extra_data(
    struct detools_apply_patch_in_place_t *self_p)
{
    return (
...
static int in_place_process_adjustment(
    struct detools_apply_patch_in_place_t *self_p)
{
    int res;
    int offset;
                             ...
                             void *arg_p)
{
    ...
    while ((patch_offset < patch_size) && (res == 0))
    {
        ...
        if (res == 0)
        {
            res = detools_apply_patch_process(apply_patch_p, &chunk[0],
                                              chunk_size);
                                  ...
                                  detools_write_t to_write, void *arg_p)
{
    ...
    res = detools_apply_patch_init(&apply_patch, from_read, from_seek,
                                   patch_size, to_write, arg_p);
...
static int in_place_callbacks_process(
    ...
    detools_read_t patch_read, size_t patch_size, void *arg_p)
{
    int res;
    size_t patch_offset;
    size_t chunk_size;
                                           ...
                                           size_t patch_size, void *arg_p)
{
    ...
    return (in_place_callbacks_process(&apply_patch, patch_read, patch_size,
                                       arg_p));
}
